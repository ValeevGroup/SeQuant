    ...
    for (size_t R = PMIN; R <= P; ++R) {
      ...
      if (get_default_context().spbasis() == SPBasis::Spinor) {
        ...
        if (type == EqnType::t) {
          ...
          // augmented_biorthogonal_transform contains post-processing step
          // (WK_biorthogonalization_filter method) which removes the redundancy
          // caused by biorthogonal transformation and gives the most compact
          // set of equations. However, we need to restore the effects of those
          // deleted terms. So, after evaluate_symm call in sequant evaluation
          // scope, we need to call biorthogonal_nns_project_<backend>.

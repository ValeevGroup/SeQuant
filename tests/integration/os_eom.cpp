...
class compute_eomcc_openshell {
 ...
 public:
  ...
  void operator()(bool print) {
    ...
    // generate spin-orbital EOM equations
    timer_pool.start(N);
    ...
    for (size_t i = 0; i < eqvec.size(); ++i) {
      ...
      // the first factor that actually is a Tensor
      auto first_tensor = [](const ExprPtr& term) -> Tensor {
        if (term->is<Tensor>()) return term->as<Tensor>();
        SEQUANT_ASSERT(term->is<Product>());
        for (const auto& factor : *term)
          if (factor->is<Tensor>()) return factor->as<Tensor>();
        throw std::runtime_error("os_eom: no Tensor factor in first term");
      };
      Tensor A = first_tensor(eqvec[i].is<Sum>() ? eqvec[i]->at(0) : eqvec[i]);
      std::wcout << "\n R[" << i << "]: A tensor (first tensor): \n"
                 << to_latex_align(ex<Tensor>(A)) << "\n";
      std::wcout << "R[" << i << "] has " << eqvec[i].size() << " terms\n";

      // open-shell spin adaptation: one expression per external spin case
      auto os_st = open_shell_CC_spintrace(eqvec[i]);
      std::wcout << "number of spin cases " << os_st.size() << "\n";

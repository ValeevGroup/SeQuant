...
// stressed. This is a small HAND-BUILT fixture (its helpers prefixed
...
// for contracted_indices()/is_product() -- build_site_of/analyze_legality
// consult them).
...
OrderedSchedFixture orderedsched_water20_auxocc_fixture() {
  ...
  // spectator batching, so the DP may open an external occ loop per node
  ...
  // MPQC passes an EMPTY mode_order (cck.ipp: the ordered driver
...
// reads off the OrderedSchedule IR. \c OrderedSchedule's \c ScopeBlock
// carries \c axis / \c kind, an explicit value_id -> \c OutputKind \c
// outputs map, and \c steps, which interleaves build steps with an ORDERED
// LIST of sibling child \c ScopeBlocks (Task 4's forced-split producer/
// consumer passes are two such siblings at ONE level). This test asserts
// that shape is actually present and walkable on both real fixtures, and
// that value_ids resolve through \c rich.cells[...].hash exactly as the \c
// build_value_node_map bridge does elsewhere. A structural assertion only --
// SP3 consumes this; SP2 does not execute anything.
// ===========================================================================
// Hidden [blocked-layers-1-2] -- encodes a design this branch does not
// implement; see the [blocked-layers-1-2] note in this file. Do not un-hide
// without rewriting the expectation first.
TEST_CASE(
  ...
  // build_value_node_map relies on

...
TEST_CASE(
  ...
  // occ loop id. Wrapped in forest root C: a forest root always escapes in
  // full, so wrapping keeps M3 itself a genuine per-iteration Transient
  // BuildStep inside the occ block (a bare root, with no further consumer,
  // would instead be forced to materialize as an AccumulateScatter escape).
  ...
  // (mirrors the [sp2-noninner] occ-outer/aux-inner fixture's M3/Y setup).

...
/// in a tensor backend. Identical to test_ordered_executor.cpp's
/// ScalarEvalExpr.
///
...
TEST_CASE(
  ...
  // forest-descent sum. Assert the map contract instead.
...
TEST_CASE(
  ...
  // RESIDENT by the ordered executor (ordered_executor.hpp) at the root
  // cache, so that one buffer is read by every consumer. Accumulating
  // (a*b + c) in
...
TEST_CASE(
  ...
  // marks persistent is never exhausted, so operand_drained() never reports
  // it drained.
...
TEST_CASE(
  ...
  SECTION("persistent, max_life == 1 -> shared") {
    ...
    ResultPtr const val = cache.store_and_access(
        node, sequant::eval_result<ResultScalar<double>>(-7.0));
    REQUIRE(val);

...
// zero-data DryRun backend), so this uses a minimal
// ScalarEvalExpr subclass (Constant/Variable leaves, Sum/Product internal
// nodes, no tensor backend) -- defined here (no
...
/// A minimal scalar-only EvalExpr for the equivalence harness.
///
...
/// The scalar-only leaf evaluator for the equivalence harness.
///
...
// take the forest-descent arm (an unconditional forward to
// sequant::evaluate(Nodes const&, layout, leaf_evaluator, cache)) -- i.e.
// the ordered branch must not disturb it when the scheduler is not set to
// ordered.
TEST_CASE(
...
// replicated (with an `orderedexec_` prefix) from
...
// kOrderedSchedWater20_pVDZF12 (job-log / mpqc PaoPnoRMP2 moments) --
// duplicated per the file-header note above.
...
TEST_CASE(
    ...
    "and the ordered executor model the same result shape, and "
  ...
  // ---- (2) forest descent (reference), SAME forest, plain CSE cache. ----
  ...
  std::wcerr << L"\n=== [ordered-executor-witness-water20] water-20 aux-only, "
             ...
             << L", forest-descent = " << fd_builds << L"\n";

  // Systematic check over EVERY root-homed (root-level BuildStep), Κ-free
  // composite -- not just the single I_node above: the ordered executor homes
  // each at the root cache and builds it exactly once.
  ...
  // ordered_executor.hpp bug to fix, NOT an assertion to weaken.
  ...
  // CSE). Both build-once executors (forest descent and ordered) keep shared
  ...
  // shape equivalence (ord vs forest-descent, above) hold:
...
// ===========================================================================
// AUX+OCC dry-run WALK gate (default-on, NOT hidden): the MPQC water-20 CSV-CCk
// residual with occ batched as an EXTERNAL index (plus the K(aux) index
// contracted and spectator batching -- the exact
// make_csv_batch_policy(occ_target>0) config) builds a well_formed schedule,
// and WALKING it end to end with the zero-data DryRun backend now completes
// with no vanished home value. The dry-run walk exercises the identical
// run_ordered_contracted_block / evaluate_impl home-read + scatter logic as a
// wet run -- only the TA tile math is stubbed -- so it pins the multi-space,
...
TEST_CASE(
  ...
  // batching ON (make_csv_batch_policy, occ_target>0), so the DP opens the
...
// const&, ...) driver entry MPQC's wet path drives) realizes the exact
...
TEST_CASE(
  ...
  // path never installs one either).
  auto const legality = sequant::eval::analyze_legality(rich, forest, policy);
  REQUIRE(legality.cells.size() == rich.cells.size());
  auto const ordered =
      sequant::eval::build_ordered_schedule(rich, legality, policy, {L"Κ"});
  ...
  // the SAME driver entry (sequant::evaluate(Nodes const&, BatchPolicy
  // const&, ...), ordered_executor.hpp) that MPQC's wet ordered
  // path drives -- the point of this test.
...
// leaf, with nothing left on that path to apply the Read's declared slice.
// So the first consumer of a leaf inside a batch loop got a whole
...
// plus a cache carrying one batch-loop context) rather than through a whole
// derived schedule: the defect is entirely in which value the leaf leg
// resolves to, and this pins it without depending on a schedule shape that
...
TEST_CASE(
  ...
  // The executor's own compute step for cell#1 (it begins the consumer and
  // reads both legs itself).
  sequant::eval::dryrun::DryRunLeafEvaluator const yield{cm};
  sequant::ResultPtr const got =
      sequant::eval::detail::compute_cell<sequant::Trace::Off>(
          node, /* cell = */ 1, resolver, yield, cache, bctx);
  REQUIRE(got);
...
// ===========================================================================
// B-full: the ordered executor computes every cell ITSELF.
//
// The walk used to hand each Build production to the generic recursive
// evaluator (evaluate_impl, eval.hpp), which then resolved the operands back
// through a probe the executor had installed on the cache. The executor now
// computes a cell directly from its table reads (detail::compute_cell): the
// operands come from CellReadResolver::fetch and the op is apply_one_op. The
// property that pins this is negative and exact -- ZERO entries into
// evaluate_impl for a whole ordered evaluation (OrderedOpCounts::probes) --
// plus the positive count that every production really did go through
// compute_cell.
//
// Two fixtures: the flat unbatched scalar forest (where the build count is
// exactly the schedule's BuildStep count, since nothing is skipped and no
// loop repeats a step) and the batched water-20 residual (where the count is
// bracketed by the cell table -- at least one production per Build cell, at
...
TEST_CASE("ordered executor computes cells through apply_one_op only",
  ...
  SECTION("unbatched scalar forest: one build per BuildStep, zero probes") {
    ...
    // No batchable index => a flat BuildStep sequence: every step is one
    // production, performed exactly once.
    std::size_t n_build_steps = 0;
    for (auto const& step : ordered.root.steps) {
      REQUIRE(std::holds_alternative<sequant::eval::BuildStep>(step.value));
      ++n_build_steps;
    }
    REQUIRE(n_build_steps > 0);

    auto cache = sequant::CacheManager<ScalarNode>::empty();
    std::function<std::size_t(Index const&)> const target =
        [](Index const&) -> std::size_t { return 1; };
    ResultPtr const got = evaluate_ordered_schedule(
        forest, ordered, rich, ScalarEvalExpr::annot_t{}, yield, cache, target);
    REQUIRE(got);
    // The numbers the equivalence case above pins, recomputed here: the
    // counts below are only meaningful for a run that is still correct.
    CHECK(got->as<ResultScalar<double>>().value() == Catch::Approx(-42.25));

    auto const counts = sequant::eval::detail::ordered_op_counts();
    CHECK(counts.probes == 0);
    CHECK(counts.builds == n_build_steps);
    CHECK(counts.assembles == 0);  // no batch loop => no Assemble step
  }

  SECTION("batched water-20 residual: assembles fire, still zero probes") {
    using sequant::eval::dryrun::EvalExprDryRun;
    using sequant::eval::dryrun::EvalNodeDryRun;
    using Node = EvalNodeDryRun;
    ...
    auto const ordered = sequant::eval::build_ordered_schedule(
        rich, legality, policy, std::initializer_list<std::wstring>{});
    REQUIRE(sequant::eval::well_formed(ordered));
    // The fixture really is batched: at least one realized loop block.
    {
    ...
    // The table the executor derives, built HERE with the same inputs (the
    // construction the [assemble-dest] case uses, validated below), so the
    // build-count bracket is stated on the very cells the run produces.
    auto const vmap = sequant::eval::build_value_node_map(forest);
    auto const sma =
        sequant::eval::compute_sliced_mode_assignment(ordered, rich);
    sequant::eval::CellTableInputs in;
    ...
    // EVERY build the executor performs is the production of a Build cell of
    // this table -- an "implicit" build (an Assemble step producing its own
    // per-batch source, no BuildStep of its own) included, since that source
    // IS a Build cell. So the count is bracketed exactly:
    //   lower -- one production per Build cell. On a COLD run (an empty
    //            persistent store, so the cache-halt seed is empty and its
    //            consumer closure adds nothing) no cell's production is
    //            elided outright; `produce_if_absent` only elides a REPEAT
    //            visit of a cell already produced once.
    //   upper -- a cell's production runs at most once per visit of its own
    //            scope, i.e. once per batch of every loop instance on its
    //            scope path.
    // The gap between the two is exactly what reuse (produce_if_absent) and
    // whole-block skipping save; pinning both ends is as exact as this can be
    // stated without re-implementing those two decisions here.
    std::size_t n_build_cells = 0, max_builds = 0;
    for (auto const& c : table.cells) {
      if (c.production.kind != sequant::eval::ProductionKind::Build) continue;
      ++n_build_cells;
      std::size_t reps = 1;
      for (auto const& [key, lat] : c.scope.path) {
        (void)lat;
        reps *= std::max<std::size_t>(1, in.n_batches_of(key));
      }
      max_builds += reps;
    }
    REQUIRE(n_build_cells > 0);
    REQUIRE(max_builds > n_build_cells);  // the fixture really does repeat

    // Assembles are bracketed the same way: one per Assemble cell at least
    // (every block output is folded on the visit that produces it), at most
    // one per batch of the assembling loop and of every loop enclosing it --
    // i.e. of the cell's own scope path plus the block's own loop, which the
    // source cell's scope path already names.
    std::size_t n_assemble_cells = 0, max_assembles = 0;
    for (auto const& c : table.cells) {
      if (c.production.kind != sequant::eval::ProductionKind::Assemble)
        continue;
      ++n_assemble_cells;
      std::size_t reps = 1;
      for (auto const& [key, lat] :
           table.cells[c.production.source].scope.path) {
        (void)lat;
        reps *= std::max<std::size_t>(1, in.n_batches_of(key));
      }
      max_assembles += reps;
    }
    REQUIRE(n_assemble_cells > 0);
    ...
    auto const run = [&]() {
      ...
      REQUIRE_NOTHROW(
          sequant::eval::evaluate_ordered_schedule<sequant::Trace::Off>(
              forest, ordered, rich, layout, yield, ordered_cache, target, {},
              is_volatile_node));
      return sequant::eval::detail::ordered_op_counts();
    };
    auto const counts = run();
    logger.eval.level = prev_level;

    INFO("builds = " << counts.builds << " (cells " << n_build_cells << ", max "
                     << max_builds << "), assembles = " << counts.assembles
                     << " (cells " << n_assemble_cells << ", max "
                     << max_assembles << "), probes = " << counts.probes);
    CHECK(counts.probes == 0);
    CHECK(counts.builds >= n_build_cells);
    CHECK(counts.builds <= max_builds);
    CHECK(counts.assembles >= n_assemble_cells);
    CHECK(counts.assembles <= max_assembles);

    // The walk is deterministic: a second run of the SAME schedule on a fresh
    // cache performs exactly the same productions, so the counts above are an
    // exact fingerprint of this schedule, not a sampled range.
    auto const again = run();
    CHECK(again.builds == counts.builds);
    CHECK(again.assembles == counts.assembles);
...
TEST_CASE("ordered executor refuses a batched schedule with no backend ops",
  ...
  // One realized batch loop holding one build: enough for
  // ordered_n_batches_by_loop to need the backend's axis chunking.
  OrderedSchedule sched;
  {
    ScopeBlock block;
    block.axis = sequant::Index{L"i_1"};
    block.latitude_ordinal = 0;
    block.steps.push_back(Step{BuildStep{0}});
    sched.root.steps.push_back(Step{std::move(block)});
    sched.num_values = 1;
  }
  std::function<std::size_t(sequant::Index const&)> const target =
  ...
  sequant::BackendArrayOps ops;
  ops.make_zeros = [](sequant::container::vector<sequant::Index> const&) {
    return sequant::ResultPtr{};
  };
  ops.axis_batches = [](sequant::Index const&, std::size_t)
      -> sequant::container::svector<std::pair<std::size_t, std::size_t>> {
    return {{0, 4}, {4, 8}};
  };
  std::function<std::size_t(sequant::eval::LoopKey const&)> n_batches;
  REQUIRE_NOTHROW(n_batches = sequant::eval::detail::ordered_n_batches_by_loop(
                      sched, target, &ops));

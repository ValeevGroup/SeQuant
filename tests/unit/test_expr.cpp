...
TEST_CASE("expr", "[elements]") {
  ...
  SECTION("power") {
    ...
    // constructor
    auto c2 = ex<Constant>(rational{1, 2});
    auto vx = ex<Variable>(L"x");
    ...
    // LaTex
    // Power::to_latex(): base_latex^exp_latex
    REQUIRE(p1.to_latex() == L"{{{\\frac{1}{2}}}}^{\\frac{1}{2}}");
    // exponent == 1 collapses to just base
    Power p_exp1(c2, rational{1});
    REQUIRE(p_exp1.to_latex() == L"{{{\\frac{1}{2}}}}");

    // Variable base
    Power pv(vx, rational{2, 1});
    REQUIRE(pv.to_latex() == L"{x}^{2}");

    // Hashing
    REQUIRE(hash::value(p1) == hash::value(p2));
    REQUIRE(hash::value(p1) != hash::value(p3));
    REQUIRE(hash::value(p1) != hash::value(p4));

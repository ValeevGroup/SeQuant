...
TEST_CASE("TEST_RPN", "[parse_expr]") {
  ...
  SECTION("add tokens: equal precedence operators") {
    rpn.reset();
    rpn.add_operand(token<Number>(2));
    rpn.add_operator(token<Plus>());
    rpn.add_operand(token<Number>(3));
    rpn.add_operator(token<Minus>());
    rpn.add_operand(token<Number>(1));
    // expression: '2 + 2/3 - 1'
    // pos-fix notation:
    //                 '2 2/3 + 1 -'
    REQUIRE(rpn.close());
  ...
  SECTION("add tokens: varying precedence operators") {
    ...
    rpn.add_operand(token<Number>(3));
    rpn.add_operator(token<Times>());
    rpn.add_operand(token<Number>(4));
    // expression: '1 * 2 + 3 - 3 * 4'
    // post-fix notation:
    //             '1 2 * 3 + 3 4 * -'
    REQUIRE(rpn.close());
    REQUIRE(rpn.tokens().at(0)->as<Number>().clone() == ex<Constant>(1));
    REQUIRE(rpn.tokens().at(1)->as<Number>().clone() == ex<Constant>(2));
    REQUIRE(rpn.tokens().at(2)->is<Times>());
    REQUIRE(rpn.tokens().at(3)->as<Number>().clone() == ex<Constant>(3));
    REQUIRE(rpn.tokens().at(4)->is<Plus>());
    REQUIRE(rpn.tokens().at(5)->as<Number>().clone() == ex<Constant>(3));
    REQUIRE(rpn.tokens().at(6)->as<Number>().clone() == ex<Constant>(4));
  ...
  SECTION("add tokens: parenthesis") {
    ...
    rpn.add_operand(token<Number>(4));
    rpn.add_operator(token<Plus>());
    rpn.add_operand(token<Number>(9));
    rpn.add_operator(token<Minus>());
    rpn.add_operand(token<Number>(10));
    REQUIRE(rpn.add_right_parenthesis());
    rpn.add_operator(token<Plus>());
    rpn.add_operand(token<Number>(2));
    REQUIRE(rpn.add_right_parenthesis());
    rpn.add_operator(token<Minus>());
    rpn.add_operand(token<Number>(12));
    // post-fix notation:
    //             '3 4 9 + 10 - * 2 + 12 -'
    REQUIRE(rpn.close());
    REQUIRE(rpn.tokens().at(0)->as<Number>().clone() == ex<Constant>(3));
    REQUIRE(rpn.tokens().at(1)->as<Number>().clone() == ex<Constant>(4));
    REQUIRE(rpn.tokens().at(2)->as<Number>().clone() == ex<Constant>(9));
    REQUIRE(rpn.tokens().at(4)->as<Number>().clone() == ex<Constant>(10));
    REQUIRE(rpn.tokens().at(7)->as<Number>().clone() == ex<Constant>(2));
    REQUIRE(rpn.tokens().at(9)->as<Number>().clone() == ex<Constant>(12));

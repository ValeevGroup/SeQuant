...
The :class:`mbpt::Operator <sequant::mbpt::Operator>` class represents an abstract `Fock-space <https://en.wikipedia.org/wiki/Fock_space>`_ many-body operator, such as the Hamiltonian operator, that can be used within SeQuant expressions.
We already saw :doc:`earlier </user/getting_started/operators>` that
...
.. math::
    \left(1 + \hat{o}_1\right)^2 = 1 + \hat{o}_1 + \hat{o}_1 + \hat{o}_1^2 = 1 + 2 \hat{o}_1 + \hat{o}_1^2
...
each of which has 1 (particle or hole) creator and 1 (particle or hole) annihilator.
...
Finally, for a product of ``mbpt::Operator`` it is possible to determine its effect on the quantum numbers of the input state using simple rules based on `Wick's theorem <https://en.wikipedia.org/wiki/Wick's_theorem>`_.
...
This logic can also be used to **check if two operators commute with each other**, which is crucial for reducing the expression to its compact canonical form.
...
Returns Lie similarity transformation, :math:`\bar{A} = e^{-\hat{B}} \hat{A} e^{\hat{B}}`, as a series of nested `commutators <https://en.wikipedia.org/wiki/Commutator>`_, :math:`[\hat{A},\hat{B}]`, :math:`[[\hat{A},\hat{B}],\hat{B}]`, etc., up to order ``r``.
...
<https://en.cppreference.com/w/cpp/language/aggregate_initialization.html#Designated_initializers>`_ --

...
int main() {
  ...
  // Traditional CCSD
  auto t_eqs = CC{2}.t();
  std::wcout << "T1 equation: " << to_latex(t_eqs[1]) << "\n"
             << "T2 equation: " << to_latex(t_eqs[2]) << "\n";

  // Lambda equations
  auto l_eqs = CC{2}.λ();
  std::wcout << "λ1 equation: " << to_latex(l_eqs[1]) << "\n"
             << "λ2 equation: " << to_latex(l_eqs[2]) << "\n";

  // Unitary CCSD
  auto Ut_eqs =
      CC{2, CC::Ansatz::U}.t(4);  // Use 4th-order commutator expansion
  std::wcout << "T1 equation (UCC): " << to_latex(Ut_eqs[1]) << "\n"
             << "T2 equation (UCC): " << to_latex(Ut_eqs[2]) << "\n";

  // Orbital-optimized CCSD
  auto oT_eqs = CC{2, CC::Ansatz::oT}.t();
  std::wcout << "T2 equation (OCC): " << to_latex(oT_eqs[2]) << "\n";
  // end-snippet-1

  // start-snippet-2
  // EE-EOM-CCSD (excitation energy)
  auto r_eqs = CC{2}.eom_r(nₚ(2), nₕ(2));
  std::wcout << "R1 equation: " << to_latex(r_eqs[1]) << "\n"
             << "R2 equation: " << to_latex(r_eqs[2]) << "\n";

  //  EE-EOM-CCSD Left eigenvectors
  auto ee_l_eqs = CC{2}.eom_l(nₚ(2), nₕ(2));
  std::wcout << "L1 equation: " << to_latex(l_eqs[1]) << "\n"
             << "L2 equation: " << to_latex(l_eqs[2]) << "\n";

  // IP-EOM-CCSD (ionization potential)
  auto ip_eqs = CC{2}.eom_r(nₚ(0), nₕ(1));
  std::wcout << "IP-R1 equation: " << to_latex(ip_eqs[1]) << "\n";

  // EA-EOM-CCSD (electron attachment)
  auto ea_eqs = CC{2}.eom_r(nₚ(1), nₕ(0));
  std::wcout << "EA-R1 equation: " << to_latex(ea_eqs[1]) << "\n";
  // end-snippet-2

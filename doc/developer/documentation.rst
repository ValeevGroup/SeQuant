...
SeQuant uses `Doxygen <https://www.doxygen.nl/>`_ and `Sphinx <https://www.sphinx-doc.org/en/master/>`_ to build documentation.
The API documentation can be generated in two modes:
...
The ``doc`` directory itself is the Sphinx source directory; the hidden ``.sphinx/`` and ``.doxygen/`` directories only hold configuration.

- ``CMakeLists.txt`` - Main build file for all documentation
- ``index.rst`` - Main index file for documentation
- ``api.rst.in`` - Template for the API reference page; configured into ``api.rst`` (gitignored) at CMake time
- ``user/`` - User-facing documentation

  - ``getting_started/`` - Installation and first steps
  - ``guide/`` - User guide for the individual features
  - ``utilities/`` - Documentation of the standalone tools under ``utilities/``

- ``developer/`` - Advanced developer documentation
- ``maintainer/`` - Maintainer documentation (release procedures, etc.)
- ``examples/`` - C++ code snippets included in the documentation; they are compiled (and, unless marked with
  ``// SEQUANT_DOC_EXAMPLE: DO NOT RUN``, run) as part of the build so that they cannot go stale
- ``images/`` - Images (e.g. logos) used by the documentation
- ``api/`` - API documentation (generated by Exhale/Breathe when ``SEQUANT_BUILD_DOCS_API_BREATHEEXHALE=ON``; gitignored)

Building Documentation
----------------------

After all dependencies are installed, you can build the documentation using ``CMake``. The documentation will be generated in ``doc/html`` under your
build directory. You can open the ``index.html`` file in your web browser to view the documentation.
...
All user-facing documentation should be added to the ``doc/user/`` directory. Internal documentation can be added to the ``doc/developer/``
directory. Code examples that are included in the documentation should be placed under ``doc/examples/`` so that they are compiled along with the
rest of SeQuant.

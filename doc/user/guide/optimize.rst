...
(e.g. minimizing intermediate storage or peak memory instead of raw flop count), common-subexpression elimination across the whole
sum, and a number of specialized knobs for batched/sliced evaluation. These are tuning parameters for advanced use cases (e.g. very
large-scale calculations with tight memory budgets) rather than everyday concepts — see the API reference for

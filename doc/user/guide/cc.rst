...
Coupled-cluster (CC) theory is one of the most accurate and widely used quantum chemistry approaches for describing electron correlation in molecular systems. It represents the many-electron wavefunction using an exponential ansatz:
...
The SeQuant ``CC`` class (see :class:`sequant::mbpt::CC`) provides a powerful symbolic algebra engine for deriving the complex equations that arise in coupled-cluster theory for both ground and excited states. It supports various formulations of the CC method, including traditional, unitary, and orbital-optimized ansätze.

Overview
--------

The ``sequant::mbpt::CC`` class is a derivation engine for generating coupled-cluster equations. It can derive:
...
The ``CC`` class supports several coupled-cluster ansätze through the ``CC::Ansatz`` enum (see :enum:`sequant::mbpt::CC::Ansatz`):

- ``Ansatz::T``: Traditional coupled-cluster ansatz, where the wavefunction is represented as :math:`|\Psi_{\text{CC}}\rangle = e^{\hat{T}}|\Phi_0\rangle`. This is the standard approach used in most implementations.
...
- ``Ansatz::U``: Unitary coupled-cluster ansatz, where the wavefunction is represented as :math:`|\Psi_{\text{UCC}}\rangle = e^{\hat{T} - \hat{T}^\dagger}|\Phi_0\rangle`. Particularly useful for quantum computing applications.
...
The following examples demonstrate how to use the ``CC`` class to derive coupled-cluster equations for various ansätze and excitation levels.

...
For UCC, the optional ``block_ranks`` argument gives the per-block truncation orders as a row-major matrix over the EOM manifolds: nested-commutator order for BCH and :math:`\bar{H}^{k}` order for Bernoulli. When omitted, every block uses the configured H̄ rank. Traditional CC does not support block ranks.
...
For unitary BCH expansions, ``Options::hbar_singles_comm_rank`` applies an additional singles-only similarity transform generated by :math:`\sigma_1 = T_1 - T_1^\dagger` after the primary expansion. A zero rank disables this transform; a positive rank requires a unitary ansatz with singles amplitudes enabled and is not supported with Bernoulli expansions.

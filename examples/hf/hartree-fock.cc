...
btas::Tensor<double> compute_mo_ints(
    const Matrix& coff_mat, const std::vector<libint2::Shell>& shells) {
  ...
  // See 'The Smarter Algorithm' at
  // http://sirius.chem.vt.edu/wiki/doku.php?id=crawdad:programming:project4

  BTensor coff_tensor(nao, nao);
  std::copy(coff_mat.data(), coff_mat.data() + coff_mat.size(),
            coff_tensor.begin());

  // first bracket (from right to left in the equation linked above)
  BTensor s_contract(nao, nao, nao, nao);
  btas::contract(1.0, ints_ao, {1, 2, 3, 4}, coff_tensor, {4, 5}, 0.0,
                 s_contract, {5, 1, 2, 3});

  // second bracket
  BTensor rs_contract(nao, nao, nao, nao);
  btas::contract(1.0, s_contract, {1, 2, 3, 4}, coff_tensor, {4, 5}, 0.0,
                 rs_contract, {5, 1, 2, 3});

  // third bracket
  BTensor qrs_contract(nao, nao, nao, nao);
  btas::contract(1.0, rs_contract, {1, 2, 3, 4}, coff_tensor, {4, 5}, 0.0,
                 qrs_contract, {5, 1, 2, 3});

  // final loop
  BTensor pqrs_contract(nao, nao, nao, nao);
  btas::contract(1.0, qrs_contract, {1, 2, 3, 4}, coff_tensor, {4, 5}, 0.0,
                 pqrs_contract, {5, 1, 2, 3});
...
TA::TArrayD compute_mo_ints(const Matrix& coff_mat,
      ...
      for (auto s2 = 0; s2 != shells.size(); ++s2) {
        ...
        for (auto s3 = 0; s3 != shells.size(); ++s3) {
          ...
          for (auto s4 = 0; s4 != shells.size(); ++s4) {
            ...
            // we don't have an analog of Eigen for tensors (yet ... see
            // github.com/BTAS/BTAS, under development) hence some manual labor
            // here: 1) loop over every integral in the shell set (= nested
            // loops over basis functions in each shell) and 2) add contribution
            // from each integral
            for (auto f1 = 0, f1234 = 0; f1 != n1; ++f1) {
              const auto bf1 = f1 + bf1_first;
              for (auto f2 = 0; f2 != n2; ++f2) {
  ...
  // create ints_ao
  TA::TiledRange trange_ao{TA::TiledRange1{0, nao}, TA::TiledRange1{0, nao},
  ...
  *(ints_ao.begin()) = tile;
  ...
  auto mat2tensor = [&](const TA::Range& range) {
    TA::Tensor<double> tile(range);
    std::copy(coff_mat.data(), coff_mat.data() + coff_mat.size(), tile.begin());
    return tile;
  };
  // transform the coefficient Eigen::MatrixD to TA::Tensor<double> type
  TA::TArrayD coeff_tensor(
      world, TA::TiledRange{TA::TiledRange1{0, nao}, TA::TiledRange1{0, nao}});
  tile = coeff_tensor.world().taskq.add(
      mat2tensor, coeff_tensor.trange().make_tile_range(0));
  *(coeff_tensor.begin()) = tile;
  ...
  ints_mo("5,1,2,3") =
      ints_mo("1,2,3,4") * coeff_tensor("4,5");  // qrs contract
  ints_mo("5,1,2,3") =
      ints_mo("1,2,3,4") * coeff_tensor("4,5");  // pqrs contract

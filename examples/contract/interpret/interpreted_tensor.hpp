...
class InterpretedTensor {
 public:
  ...
  void link_tensor(const std::shared_ptr<T>&);
...
InterpretedTensor<T>::InterpretedTensor(const std::wstring& lbl,
  ...
  auto size = ov.size();
  if (size % 2 != 0) throw "The dimension of bras and kets should match!";

  using std::back_inserter;
  using std::copy;
  // assumes the function parameter 'ov' is a sorted vector
  copy(ov.begin(), ov.begin() + size / 2, back_inserter(this->bra_indices_));
  copy(ov.begin() + size / 2, ov.end(), back_inserter(this->ket_indices_));
  //
  this->label_ = lbl;
}
...
template <typename T>
auto& InterpretedTensor<T>::tensor() {
  return *(this->tensor_);
}
    ...
    for (const auto& i : idx_container) {
      ...
      if (oclass == -1)  // -1 = occupied
        ...
        throw std::domain_error(
            "Neither occupied nor unoccupied orbital encountered");
    }
  };
...
template <typename T>
void InterpretedTensor<T>::link_tensor(const std::shared_ptr<T>& t) {
  this->tensor_ = t;
}
...
#ifdef SEQUANT_HAS_BTAS
  btas::scal(d, this->tensor());
#endif
#ifdef SEQUANT_HAS_TILEDARRAY
  TA::scale(this->tensor(), d);
#endif
}

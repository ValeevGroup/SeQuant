...
template <typename T>
InterpretedTensor<T> eval_equation(
    const sequant::ExprPtr&, const std::map<std::wstring, std::shared_ptr<T>>&);
...
template <typename T>
InterpretedTensor<T> eval_product(
    const sequant::ExprPtr&, const std::map<std::wstring, std::shared_ptr<T>>&);

template <typename T>
InterpretedTensor<T> eval_sum(
    const sequant::ExprPtr&, const std::map<std::wstring, std::shared_ptr<T>>&);
...
std::vector<Perm> perm_calc(std::vector<size_t>, size_t,
...
#ifdef SEQUANT_HAS_BTAS
btas::Tensor<double> core_contract(
    double,                       // scalar
...
#ifdef SEQUANT_HAS_TILEDARRAY
TA::TArrayD core_contract(double, const TA::TArrayD&,
                          const std::vector<size_t>&, const TA::TArrayD&,
...
#ifdef SEQUANT_HAS_TILEDARRAY
TA::TArrayD core_sum(const TA::TArrayD&, const TA::TArrayD&,
                     bool subtract = false);
#endif
...
InterpretedTensor<T> eval_equation(
    ...
    const std::map<std::wstring, std::shared_ptr<T>>& tmap) {
...
btas::Tensor<double> antisymmetrize(const btas::Tensor<double>& tensor) {
  ...
  for (const auto& p : vp) {
    for (const auto& q : vp) {
      ...
      for (auto qq : q.perm) perm_vec.push_back((size_t)rank / 2 + qq);
...
InterpretedTensor<T> contract(const InterpretedTensor<T>& t1,
  ...
  auto gen_ords_vec = [&index_to_ordinal](const InterpretedTensor<T>& t) {
    ords_vec v;
    for (const auto& i : t.bras()) v.push_back(index_to_ordinal.at(i));

    for (const auto& i : t.kets()) v.push_back(index_to_ordinal.at(i));
    return v;
  };
...
#ifdef SEQUANT_HAS_BTAS
  result.link_tensor(std::make_shared<btas::Tensor<double>>(core_contract(
          scal, t1.tensor(), t1_ords, t2.tensor(), t2_ords, nc_ords)));
#endif

#ifdef SEQUANT_HAS_TILEDARRAY
  result.link_tensor(std::make_shared<TA::TArrayD>(core_contract(
          scal, t1.tensor(), t1_ords, t2.tensor(), t2_ords, nc_ords)));
#endif
  return result;
...
InterpretedTensor<T> contract_vec(const std::vector<InterpretedTensor<T>>& vct,
  ...
  if (i == 0) {
    if (scal != 1.0) {
      auto tmp = vct[i];
      auto temp_tnsr = scale(scal, vct[i].tensor());
#ifdef SEQUANT_HAS_BTAS
      auto tmp_tnsr = std::make_shared<btas::Tensor<double>>(temp_tnsr);
#endif
#ifdef SEQUANT_HAS_TILEDARRAY
      auto tmp_tnsr = std::make_shared<TA::TArrayD>(temp_tnsr);
#endif
      tmp.link_tensor(tmp_tnsr);
      return tmp;
    }
    return vct[i];
  } else if (i == 1)
  ...
  for (const auto& f : p.factors()) {
    if (!f->is<sequant::Tensor>())
      ...
      if (t.label() != L"A") {
        ...
        ip_tensor.link_tensor(
            tmap.find(ip_tensor.label() + L"_" + ip_tensor.translate())
                ->second);
  ...
  for (auto i = 0; i < smands.size(); ++i) {
    if (!smands.at(i)->is<sequant::Tensor>())
      ...
      if (t.label() != L"A") {  // 'A' tensors are omitted
        ...
        svec[i].link_tensor(
            tmap.find(svec[i].label() + L"_" + svec[i].translate())->second);
      }
    }
  }
  auto result = svec[0];
...
#ifdef SEQUANT_HAS_BTAS
  auto t = std::make_shared<btas::Tensor<double>>(
      core_sum(s1.tensor(), s2.tensor(), subtract));
#endif
#ifdef SEQUANT_HAS_TILEDARRAY
  auto t = std::make_shared<TA::TArrayD>(
      core_sum(s1.tensor(), s2.tensor(), subtract));
#endif
...
btas::Tensor<double> core_contract(
  ...
  btas::contract(scal, t1, t1_ords, t2, t2_ords, 0.0, result, nc_ords);
  return result;
}
#endif

#ifdef SEQUANT_HAS_TILEDARRAY
TA::TArrayD core_contract(double scal, const TA::TArrayD& t1,
  ...
  result(ords_to_csv_str(nc_ords)) =
      scal * t1(ords_to_csv_str(t1_ords)) * t2(ords_to_csv_str(t2_ords));
  return result;
}
#endif
...
#ifdef SEQUANT_HAS_TILEDARRAY
TA::TArrayD core_sum(const TA::TArrayD& t1, const TA::TArrayD& t2,
                     bool subtract) {
  if (!t2.is_initialized()) return t1;

...
int main(int argc, char* argv[]) {
...
#if 1
  Tensor g1 = Tensor(L"g", {Index{L"i_1"}, Index{L"i_2"}},
                    {Index{L"a_1"}, Index{L"a_2"}}, Symmetry::antisymm);
  // Tensor g2 = Tensor(L"g", {Index{L"a_1"}, Index{L"a_3"}},
  //                   {Index{L"i_1"}, Index{L"i_2"}}, Symmetry::antisymm);
  // Tensor T1 = Tensor(L"t",{Index{L"i_10"}}, {Index{L"a_10"}}, Symmetry::antisymm);
  ...
  Product expression{};
  {
    ExprPtr g1_ptr = std::make_shared<Tensor>(g1);
    //ExprPtr g2_ptr = std::make_shared<Tensor>(g2);

//    ExprPtr t1_ptr = std::make_shared<Tensor>(T1);
    ExprPtr t2_ptr = std::make_shared<Tensor>(T2);
//    ExprPtr t3_ptr = std::make_shared<Tensor>(T3);
//    ExprPtr t4_ptr = std::make_shared<Tensor>(T4);

    expression.append(0.25, g1_ptr);
//     expression.append(1, g2_ptr);
//    expression.append(1, t1_ptr);
...
//    expression.append(1,t4_ptr);
  ...
  // auto function_check = spintrace(temp_expr,{{Index{L"a_2"}, Index{L"a_3"}}});

...
void try_main() {
  ...
  const auto cumulant =ex<Tensor>(L"\\gamma", WstrList{L"a_1"}, WstrList{L"i_1"});
 // const auto a =ex<Tensor>(L"a",WstrList{L"i_2", L"i_3"},WstrList{L"a_2", L"a_3"});
  ...
  auto a1 = ex<FNOperator>(std::initializer_list<Index>({Index(L"i_1")}), std::initializer_list<Index>({Index(L"a_1")}));
  ...
  auto two_body_cumu = ex<Tensor>(L"\\gamma", WstrList{L"a_2", L"a_3"}, WstrList{L"i_2", L"i_3"});
  ...
  auto cumu3 = ex<Tensor>(L"\\gamma", WstrList{L"a_1", L"a_2", L"a_3"}, WstrList{L"i_1", L"i_2", L"i_3"});
  std::wcout << " y3 " << to_latex_align(cumu3) << std::endl;
  antisymmetrize _cumu3(cumu3);
  std::wcout << to_latex_align(_cumu3.result) << std::endl;

  auto antisymm_test = ex<Constant>(-1) * _cumu1_cumu2.result + _a1_cumu2.result + ex<Constant>(-2) * _a1_cumu1_cumu2.result + ex<Constant>(4) * _cumulant_3x.result + _a_cumulant.result; // expression from Cite as: J. Chem. Phys. 127, 104107 (2007); https://doi.org/10.1063/1.2761870 eqn 26.
...
int main(int argc, char* argv[]) {
  ...
  sequant::set_default_context(SeQuant(Vacuum::Physical, IndexSpaceMetric::Unit, BraKetSymmetry::conjugate ,SPBasis::spinfree));

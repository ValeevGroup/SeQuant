...
ExprPtr T_(std::size_t Nbra, std::size_t Nket, bool complete_unoccupieds,
  ...
  return Op(OpType::t, Nbra, Nket_)(complete_unoccupieds, antisymm);
}
...
ExprPtr Lambda_(std::size_t Nbra, std::size_t Nket, bool complete_unoccupieds,
  ...
  const auto Nket_ =
      Nket == std::numeric_limits<std::size_t>::max() ? Nbra : Nket;
  return Op(OpType::l, Nbra, Nket_)(complete_unoccupieds, antisymm);
}
...
class op_impl {
 ...
 public:
  ...
  void operator()(ExprPtr& result, bool complete_unoccupieds = false,
    ...
    if (op_ == OpType::t)
      result = result
                   ? result + T_(nbra_, nket_, complete_unoccupieds, antisymm)
                   : T_(nbra_, nket_, complete_unoccupieds, antisymm);
    else if (op_ == OpType::l)
      result = result ? result + Lambda_(nbra_, nket_, complete_unoccupieds,
                                         antisymm)
                      : Lambda_(nbra_, nket_, complete_unoccupieds, antisymm);
    else
...
ExprPtr R12(IndexSpace::Type gg_space, int ansatz, bool antisymm) {
  assert(ansatz == 1 || ansatz == 2);
  return Op(OpType::R12, 2, 2)(ansatz == 2 ? IndexSpace::complete_unoccupied
...
ExprPtr R(std::size_t Nbra, std::size_t Nket, bool complete_unoccupieds,
  ...
  const auto Nket_ =
      Nket == std::numeric_limits<std::size_t>::max() ? Nbra : Nket;
  assert(Nbra > 0 || Nket_ > 0);
  return Op(OpType::R, Nbra, Nket_)(complete_unoccupieds, antisymm);
}

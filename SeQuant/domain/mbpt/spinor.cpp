...
/// the term with the external pair (p,q) exchanged everywhere, proto bundles
/// included; scalar preserved
ExprPtr external_swap_image(const ExprPtr& term, const Index& p,
                            const Index& q) {
  ...
  term->visit(
      [&all](const ExprPtr& e) {
        ...
        for (const auto& idx : e->as<Tensor>().const_indices()) all.insert(idx);
      },
      /* atoms_only = */ true);
  container::map<Index, Index> repl;
  ...
  container::svector<ExprPtr> factors;
  if (term->is<Product>()) {
    out->scale(term->as<Product>().scalar());
...
/// canonical form of a term split into its scalar and its scalar-free network
struct CanonicalKey {
  Product::scalar_type scalar{1};
  ExprPtr network;  // Product with unit scalar, or a Tensor
  bool valid = false;
};

CanonicalKey canonical_key(const ExprPtr& term,
                           const CanonicalizeOptions& opts) {
  CanonicalKey key;
  auto e = term->clone();
  // canonicalize() leaves index tags behind; a stale tag changes the result
  e->visit(
      [](const ExprPtr& x) {
        if (x->is<Tensor>()) x->as<Tensor>().reset_tags();
      },
      /* atoms_only = */ true);
  e = canonicalize(e, opts);
  if (!e) return key;
  if (e->is<Tensor>()) {
    key.network = e;
    key.valid = true;
    ...
    key.scalar = p.scalar();
    if (key.scalar == Product::scalar_type{0}) return key;
    auto net = std::make_shared<Product>();
    for (const auto& f : p.factors()) net->append(1, f, Product::Flatten::No);
    key.network = net;
    key.valid = true;
  }
  return key;
...
StrippedAntisymmetrizerFold fold_stripped_antisymmetrizer(
  ...
  StrippedAntisymmetrizerFold res;
  res.expr = block;
  if (!block || groups.empty()) {
    res.n_in = res.n_out =
        block ? (block->is<Sum>() ? block->as<Sum>().size() : 1) : 0;
    return res;
  }
  opts.ignore_named_index_labels =
      CanonicalizeOptions::IgnoreNamedIndexLabel::No;

  container::svector<ExprPtr> terms;
  if (block->is<Sum>())
    for (const auto& t : block->as<Sum>().summands()) terms.push_back(t);
  else
    terms.push_back(block);
  res.n_in = terms.size();

  // canonical keys of every term, bucketed by network hash
  container::svector<CanonicalKey> keys;
  keys.reserve(terms.size());
  container::map<std::size_t, container::svector<std::size_t>> buckets;
  for (std::size_t i = 0; i < terms.size(); ++i) {
    keys.push_back(canonical_key(terms[i], opts));
    if (keys.back().valid)
      buckets[keys.back().network->hash_value()].push_back(i);
  }
  auto find_match = [&](const CanonicalKey& k) -> std::optional<std::size_t> {
    if (!k.valid) return std::nullopt;
    auto it = buckets.find(k.network->hash_value());
    if (it == buckets.end()) return std::nullopt;
    for (auto j : it->second)
      if (*keys[j].network == *k.network) return j;
    return std::nullopt;
  };

  // the nontrivial elements of the group generated by the swaps: every
  // nonempty subset of the generators, parity (-1)^|subset|
  const std::size_t ng = groups.size();
  SEQUANT_ASSERT(ng < 16);
  using scalar_type = Product::scalar_type;
  container::svector<bool> consumed(terms.size(), false);
  auto out = std::make_shared<Sum>();
  for (std::size_t i = 0; i < terms.size(); ++i) {
    if (consumed[i]) continue;
    consumed[i] = true;
    if (!keys[i].valid) {  // not a flat term we can key: keep verbatim
      out->append(terms[i]);
      continue;
    }
    scalar_type factor{1};
    bool dead = false;
    for (std::size_t subset = 1; subset < (std::size_t{1} << ng) && !dead;
         ++subset) {
      ExprPtr im = terms[i];
      int parity = 1;
      for (std::size_t g = 0; g < ng; ++g)
        if (subset & (std::size_t{1} << g)) {
          im = external_swap_image(im, groups[g].first, groups[g].second);
          parity = -parity;
        }
      const auto s = scalar_type{parity};
      auto k = canonical_key(im, opts);
      auto j = find_match(k);
      if (!j) continue;
      // g(T_i) = (k.scalar / keys[i].scalar) · T_i   (self-image)
      // T_j    = (keys[j].scalar / k.scalar) · g(T_i)
      if (*j == i) {
        const auto r = k.scalar / keys[i].scalar;
        if (r != s) dead = true;  // (1-P) annihilates it
        continue;
      }
      if (consumed[*j]) continue;
      const auto r = keys[*j].scalar / k.scalar;
      factor += s * r;
      consumed[*j] = true;
      ++res.n_merged;
    }
    if (dead || factor == scalar_type{0}) {
      ++res.n_dead;
      continue;
    }
    if (factor == scalar_type{1}) {
      out->append(terms[i]);
    } else if (terms[i]->is<Product>()) {
      auto p = std::make_shared<Product>(terms[i]->as<Product>());
      p->scale(factor);
      out->append(p);
    } else {
      auto p = std::make_shared<Product>();
      p->append(1, terms[i], Product::Flatten::No);
      p->scale(factor);
      out->append(p);
    }
  }
  res.n_out = out->size();
  res.expr = res.n_out == 0 ? ex<Constant>(0) : ExprPtr(out);
  return res;
}

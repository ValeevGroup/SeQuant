...
FlopsCounterCached::FlopsCounterCached(size_t no, size_t nv,
                                       const container::set<size_t>& imeds)
    : counter{no, nv}, imed_hashes{imeds} {}

size_t FlopsCounterCached::operator()(
...
STOResult single_term_opt(Product const& prod, size_t nocc, size_t nvirt,
  ...
  const auto counter = FlopsCounterCached{nocc, nvirt, imeds_hash};

  auto result = STOResult{std::numeric_limits<size_t>::max(), {}};
...
utils::binary_node<utils::eval_expr> optimize(const ExprPtr& expr,
  ...
  if (expr->is<Product>()) {
    ...
    auto summands = *expr | ranges::views::transform([canonize](auto const& s) {
      auto node = optimize(s, canonize);
      return utils::binarize_expr(node);
    }) | ranges::to_vector;

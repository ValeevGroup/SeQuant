...
class antisymm_element {
 ...
 private:
  std::vector<IndexGroup>
      index_group;  // where each tensor begins and ends. assumes particle conserving ops. needed to keep track of canonical ordering
  std::vector<std::pair<int, std::vector<Index>>>
      unique_bras_list;  // list of unique bra orderings with associated integer for the sign
  std::vector<std::pair<int, std::vector<Index>>>
      unique_kets_list;  // list of unique ket orderings with associated integer for the sign
  ExprPtr current_product;  // used to keep track of the original expression recieved by the constructor

  // generates all possible permutations while observing the canonical ordering of each tensor/operator function kept general to work with other data types, algorithm does not require sequant::Index objects
  ...
  std::vector<std::pair<int, std::vector<T>>> gen_antisymm_unique(
      ...
      while (true) {
        ...
        if (*--i < *i1) {
          ...
          nswaps += (std::distance(i1, last)) /
                    2;  // logic from https://en.cppreference.com/w/cpp/algorithm/reverse
    ...
    while (do_next_perm) {
      ...
      auto is_canonical_sign =
        ...
        for (auto& group :
             this->index_group) {  // There is only one sorted possibility in a set (tensor) considering that no index label should be the same.
 ...
 public:
  // takes a sequant::ExprPtr and generates all antisymmetric unique permutations of that expression. requires that ex_ is a product expression at this point
  ...
  antisymm_element(ExprPtr ex_) {
    ...
    auto summand_exists =
        [&new_sum](ExprPtr ex) {  // check whether a summand has already been generated to screen out same terms.
          bool value = false;
          for (auto summand = new_sum->begin_subexpr();
               summand != new_sum->end_subexpr(); summand++) {
            value = ex.get()->as<Product>() ==
                    summand->get()
                        ->as<Product>();  // ensure that this equality is mathematical and not hash based.
            if (value == true) {
              return value;
            }
          }
    ...
    for (int i = 0; i < unique_bras_list.size(); i++) {
      for (int j = 0; j < unique_kets_list.size(); j++) {  // product level
        ...
        if (!summand_exists(
                new_product)) {  // since products are canonicalized, repeats can be found.
...
class antisymmetrize {
 public:
  ...
  antisymmetrize(ExprPtr s) {
    if (s->is<Sum>()) {
      for (auto&& product : s->as<Sum>().summands()) {  // for each element in the sum
        if(product->is<Product>()) {
          antisymm_element ele(product);  // calculate the sum of all the valid permutations for each term. each object here should only live until this loop ends.
...
namespace antisymm {
  ...
  int num_closed_loops(
      std::vector<Index> init_upper, std::vector<Index> init_lower, std::vector<Index> new_upper, std::vector<Index> new_lower) {
    int result = 0;
      ...
      if (!where_same_ele(new_lower[i], in_loop).second) {
        ...
        while (!closed) {
          ...
          if (!where_exist.second) {  // if the upper particle index originally connected to the lower index in question is not part of a contraction, there is no loop with these.
            ...
            lower_index =new_lower[where_exist.first];  // the lower index below the found upper index
      ...
      for (auto&& factor : product->as<Product>().factors()){
        ...
        if (factor->is<Tensor>()){
          ...
          if(new_pairs > og_pairs){
            factor = ex<Constant>(-1) * ex<Tensor>(factor->as<Tensor>().label(),current_lower, current_upper);
          }
        }
          ...
          if(new_pairs > og_pairs){
            factor = ex<Constant>(-1) * ex<FNOperator>(current_upper, current_lower);
          }
        }
      }
    }
  ...
  // @param1 an antisymm_element object strictly so the original ordering of the indices is known
  // @param2 bool singlet_state? the looping rules and contraction prefactors are a direct result of the singlet state approximation to densities.
  // TODO: use a generalized spin summation for non-singlet states
  ExprPtr spin_sum(std::vector<Index> original_upper,std::vector<Index> original_lower, ExprPtr expression, bool singlet_state = true) {
    if (singlet_state) {
      //std::wcout << "before spin sum! :" << std::endl << to_latex_align(expression) << std::endl;
      ...
      for (auto&& product : expression->as<Sum>().summands()) {
        auto prefactor = ex<Constant>(0.125); //each term in the 3 body decomp, should have (1 /2^n) prefactor where n is rank of product so product rank always 3 for 3 body decomp
        std::vector<Index> new_upper;
        std::vector<Index> new_lower;
        for (auto&& factor : product->as<Product>().factors()) {
          if (factor->is<Tensor>() && factor->as<Tensor>().label() == L"\\gamma") {
            //prefactor = ex<Constant>(-0.5) * ex<Constant>(factor->as<Tensor>().rank()) * prefactor;
            ...
            factor = ex<Tensor>(L"\\Gamma",factor->as<Tensor>().bra(), factor->as<Tensor>().ket());
          }
          else if (factor->is<FNOperator>()) {
            //prefactor = ex<Constant>(-0.5) * ex<Constant>(factor->as<Tensor>().rank()) * prefactor;
            for (int i = 0; i < factor->as<FNOperator>().rank(); i++) {
              new_upper.push_back(factor->as<FNOperator>().creators()[i].index());
              new_lower.push_back(factor->as<FNOperator>().annihilators()[i].index());
            }
          }
        }

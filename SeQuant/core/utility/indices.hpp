...
Container<Group<SlottedIndex>> external_indices(const Expr& expr) {
  ...
  if (expr.is<Tensor>()) {
    ...
    if (is_symm || tensor.label() == reserved::antisymm_label()) {
      ...
      // For number-non-conserving, antisymmetrizer has bra_rank != ket_rank,
      // an unpaired index at a position has no partner to swap with, flip its
      // slot type instead to keep the bra<->ket conjugation.

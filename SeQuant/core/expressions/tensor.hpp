...
/// the Klein four-group generated by elementwise complex conjugation and
/// bra<->ket transposition (see the algebra section of Tensor's documentation)
enum class ValueModifier : std::uint8_t {
  None = 0,
  /// elements complex-conjugated, slots as written: `T^*{p;q} = conj(T{p;q})`
  Conjugate = 1,
  /// slots read in exchanged bra/ket roles: `T^T{q;p} = T{p;q}`
  Transpose = 2,
  /// both, i.e. the conjugate transpose: `T⁺{q;p} = conj(T{p;q})`
  Adjoint = 3
};
...
class Tensor : public Expr, public AbstractTensor, public MutatableLabeled {
 ...
 public:
  ...
  /// @return whether this tensor's slots are read in exchanged bra/ket roles
  bool transposed() const { return transposed_; }
  /// @return the value modifier composed from conjugated() and transposed()
  ValueModifier value_modifier() const {
    return static_cast<ValueModifier>((conjugated_ ? 1 : 0) |
                                      (transposed_ ? 2 : 0));
  }
  /// @return label() followed by the adjoint mark '⁺' when value_modifier()
  ///         is ValueModifier::Adjoint. This is the string label() returned
  ///         while the mark lived in the label; printing, hashing and graph
  ///         colouring use it so that adjoint tensors keep their spellings,
  ///         hashes and canonical forms.
  std::wstring decorated_label() const {
    std::wstring result(label_);
    if (value_modifier() == ValueModifier::Adjoint)
      result.push_back(sequant::adjoint_label);
    return result;
  }
  /// @brief overwrites the modifier bits without touching the slots. For
  /// rebuilds that copy a tensor's identity onto new slots (contrast
  /// conjugate()/adjoint(), which APPLY an operation).
 ...
 private:
  ...
  /// value modifier bits, see ValueModifier; both are kept out of label_ so
  ...
  hash_type memoizing_hash() const override {
    auto compute_hash = [this]() {
      ...
      // decorated_label() == the pre-modifier-bits label_, so None and
      // Adjoint hash exactly as before
      ...
      switch (value_modifier()) {
        case ValueModifier::Conjugate:
          hash::combine(val, true);  // as when conjugated_ was the only bit
          break;
        case ValueModifier::Transpose:
  ...
  bool static_less_than(const Expr &that) const override {
    ...
    if (this->value_modifier() != that_cast.value_modifier()) {
      return this->value_modifier() < that_cast.value_modifier();
    }

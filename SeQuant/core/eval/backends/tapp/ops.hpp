...
/// RAII wrapper for TAPP_executor.
...
/// RAII wrapper for TAPP_tensor_product.
struct ProductGuard {
...
/// RAII wrapper for TAPP_tensor_info.
...
/// Create a scalar (0-mode) TAPP_tensor_info.
template <typename T>
...
/// Compute the extents of the result tensor from a contraction.
/// Indices appearing in idx_result but not contracted are "free" indices.
/// Their extents come from A or B.
...
///
/// \brief Tensor contraction using the TAPP API.
///
/// Computes: result[idx_result] = alpha * A[idx_A] * B[idx_B]
///                               + beta * result[idx_result]
///
/// When beta == 0 and the result tensor is empty, it is automatically
/// allocated with the correct shape inferred from the index labels and
/// the operand extents.
///
template <typename T, typename Alloc>
...
///
/// \brief Permute a tensor using the TAPP API.
///
/// Computes: dst[dst_idx] = src[src_idx]
/// where src_idx and dst_idx define the index correspondence.
///
template <typename T, typename Alloc>
...
///
/// \brief Scale a tensor in place: tensor *= alpha.
///
template <typename T, typename Alloc>
...
///
/// \brief Dot product of two tensors (full contraction).
///
/// \return The sum of element-wise products of A and B.
/// \pre A and B must have the same shape.
///
template <typename T, typename Alloc>
...
///
/// \brief Compute the dot product with complex conjugation: sum(conj(A)*B).
///
template <typename T, typename Alloc>

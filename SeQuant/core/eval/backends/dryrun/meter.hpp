...
///
/// This struct lives here (rather than only in the test) because \c meter()
/// and MPQC's dry-run entry point consume it.
struct CacheConfig {
...
///
/// \brief Runs the policy-selected executor (forest descent or ordered, per
/// \p policy.scheduler) over \p forest through the

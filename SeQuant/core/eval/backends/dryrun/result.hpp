...
/// Whether any index of \p idx carries proto-indices, i.e. whether a result
/// over \p idx is nested.
...
/// \p a overlaid with \p b: every position of \p b wins, the rest come from
/// \p a.
...
/// model) regardless of which concrete DryRun type \p r is. \c is<T>() /
/// \c as<T>() are public Result methods, so no friendship is needed; these
/// are declared here and defined below, after both concrete classes, because
/// their bodies need those classes' definitions.
[[nodiscard]] container::svector<Index> indices_of(Result const& r);
/// Positional extent overrides of \p r's sliced modes.
...
/// slicing defect: the wet backend hangs or asserts on it, and this check
/// turns it into a dry-run failure. Throws with both ranges named.
...
struct DryRunOps {
  ...
  [[nodiscard]] static ResultPtr prod(
    ...
    // overrides feed the same CostModel closures every other dry-run size query
...
/// space's natural \c CostModel extent), and \c axis_batches partitions an
/// axis's full space extent exactly

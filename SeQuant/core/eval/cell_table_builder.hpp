...
[[nodiscard]] inline CellTable build_cell_table(CellTableInputs const& in) {
  ...
  // listed values (kind, scope path, sliced positions, partial_over), and
  // every occurrence layout and table read of them.
  if (char const* dv = detail::dump_value("SEQUANT_DUMP_CELLS_OF")) {
    auto const want = detail::dump_cells_of_values(dv);
    detail::dump_cells_of(st.table, *in.rich, want);
    detail::dump_occs_of(st.table, *in.rich, want);
  }

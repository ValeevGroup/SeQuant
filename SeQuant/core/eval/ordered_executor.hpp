...
/// The \c CellScope of the current point in the schedule walk --
...
///
/// The op itself is \c apply_one_op_traced (the shaped-product hook, the
/// recompute tally and the per-op trace event included), or \c
/// sum_in_place_traced for an accumulating \c Sum whose left operand the
...
/// \c run_ordered_schedule_pre_results call (test-facing; per-thread).
...
/// run_ordered_schedule_pre_results call (test-facing; per-thread).
...
/// produced is read from its cell rather than re-descended into -- see \c
/// compute_cell's doc comment) and one value per root-level \c ScopeBlock
/// (Contracted or External alike) via \c detail::run_ordered_contracted_block
/// (a realized batch loop, its own \c BuildStep's/nested child blocks
/// run per batch on a scratch cache, its \c AccumulateSum outputs summed
/// across batches; its \c AccumulateScatter outputs written into a
...
/// forest roots' results into the final \c ResultPtr via the shared
/// root-combine loop \c combine_forest_roots (permute-to-\p layout +
/// cross-root \c add_inplace, with the Term/Permute/SumInplace trace
/// bookkeeping).
///
...
/// \param cache The cache handle this run hangs its hooks off (backend
///        array-ops, the peak monitor, the persistent value store); the
///        table, not this cache, is where a value repeated across \c
///        BuildStep's operand needs is deduped.
...
/// LegalitySchedule (\c analyze_legality) and the \c OrderedSchedule (\c
/// build_ordered_schedule) from \p forest's own placement -- the \c
/// node_slice_mask() annotations a prior factorizer pass (e.g. \c optimize()
/// with a batched objective, or a test that stamps them directly) already
/// recorded on \p forest's nodes -- then runs the ordered executor with \p
/// policy.batch_target_size as the batch-partition source and \p
/// make_scope_guard forwarded verbatim. The throwaway \c
/// eval::dryrun::CostModel / \c SizeRegime built here are inert for \c
/// compute_dag_boulevard (its \c cm parameter is unused, kept only for
/// signature symmetry -- see its doc comment); they carry none of \p policy's
/// or the real backend's sizing information.
///
/// The runtime operand reads of the ordered executor are table-driven
/// (\c CellReadResolver, cell_registry.hpp), built inside \c
/// detail::run_ordered_schedule_pre_results from this same schedule's \c
/// compute_sliced_mode_assignment -- so they are correctly built and wired
/// regardless of caller; there is no separate runtime slice-inference seam.
///
...
/// \param make_scope_guard Backend scope-guard factory, forwarded to
///        \c evaluate_ordered_schedule verbatim.
///
template <Trace EvalTrace = Trace::Default, meta::can_evaluate_range Nodes,
          typename F, typename N, bool FHC,
          typename ScopeGuardFactory = ::sequant::make_no_scope_guard>
...
///
/// \param mode_order Ranks the canonical loop-chain order (outermost first);
///        only consulted on the ordered arm. See \c eval::evaluate_ordered.
/// \param make_scope_guard Backend scope-guard factory; only consulted (and
///        only constructed) on the ordered arm.
///
template <Trace EvalTrace = Trace::Default, meta::can_evaluate_range Nodes,
          typename F, typename N, bool FHC,

...
ExprPtr EvalExpr::denoted_expr() const {
  ...
  // Kramers flip (flavors back to as-written). The marker is what is left
  // of the conj bit once the two channels that came WITH a conj are taken
  // out: every swap in the transform (an adjoint label, or the braket fold
  // of a Conjugate tensor) and the Kramers fold
  ...
  const bool marked = (tr.conj != tr.braket_swap) != kramers_folded_;
  if (marked) t.conjugate();

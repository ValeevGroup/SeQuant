...
EvalSequence single_term_opt(
    ...
    container::vector<NodeBatchAnnotation>* out_axes = nullptr) {
  ...
  auto const& inner_pow = cost.inner_pow;
  bool const batch_persistent_only = cost.batch_persistent_only;
  ...
  if constexpr (Metric == ObjectiveFunction::DenseSpaceTime ||
    ...
    // a mode is batchable in a role only if that role's predicate admits it.
    model.is_batchable_contracted_index = is_batchable_contracted_index;
    model.is_batchable_external_index = cost.is_batchable_external_index;
    // charge_batch_recompute defaults to true (see PeakBatchedModel): the batch
    // recomputation cost is always reflected on the flops/exec mode.
    if (out_axes) {
      auto [seq, modes] = run_single_term_opt_axes(model, network, tidxs);
      *out_axes = std::move(modes);
      return seq;
    }
    return run_single_term_opt(model, network, tidxs);
...
/// @note The remaining parameters (\c subnet_cse, \c cost) are forwarded

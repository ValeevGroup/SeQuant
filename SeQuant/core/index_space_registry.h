...
// An IndexSpaceRegistry is a context specific, modifiable, list of registered IndexSpaces.
// It is the task of the USER to properly construct a complete set for their needs.
// Thus, the IndexSpaceRegistry is the way a user can retrieve IndexSpaces specific to their context.
// because set operations(overlap, union) are context dependent, these functions must also live here.
// Currently, there are two important restrictions, 1. unified spaces must be connected (1010 invalid typeattr).
// 2. IndexSpaces with typeattr corresponding to occupied indices will always be lower than typeattr corresponding to
// unoccupied orbitals.  In the future, these restrictions may be lifted if a need arises.
//
class IndexSpaceRegistry{
 public:
  ...
  bool valid_bitop( const IndexSpace &i1, const IndexSpace &i2, const std::function<int32_t(int32_t,int32_t)> &op) {

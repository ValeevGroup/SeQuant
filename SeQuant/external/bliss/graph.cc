...
void AbstractGraph::update_labeling(unsigned int* const labeling) {
  ...
  for (unsigned int i = 0; i < N; i++, ep++) labeling[*ep] = i;
}
...
void AbstractGraph::reset_permutation(unsigned int* perm) {
  const unsigned int N = get_nof_vertices();
  for (unsigned int i = 0; i < N; i++, perm++) *perm = i;
}
...
void AbstractGraph::cert_add(const unsigned int v1, const unsigned int v2,
  ...
  if (refine_compare_certificate) {
    ...
    if ((refine_equal_to_first == false) and (refine_cmp_to_best < 0)) return;
  }
...
std::vector<bool>& AbstractGraph::long_prune_get_fixed(
    const unsigned int index) {
...
std::vector<bool>& AbstractGraph::long_prune_get_mcrs(
    const unsigned int index) {
...
void AbstractGraph::search(const bool canonical, Stats& stats) {
  ...
  if (opt_use_long_prune) long_prune_init();
  ...
  while (!search_stack.empty()) {
    ...
    /* Update statistics */
    if (p.is_discrete()) stats.nof_leaf_nodes++;
    ...
    if (p.is_discrete() and first_path_info.empty()) {
      ...
      if (opt_use_failure_recording) failure_recording_hashes.resize(base_size);
        ...
        if (p.nof_discrete_cells() ==
          ...
          if (first_path_info.empty() or child_node.fp_cert_equal) {
            if (cep.first_checked == false) {
              ...
              if (old_info.fp_on) goto handle_first_path_automorphism;
            }
          }
          ...
          if (canonical and !first_path_info.empty() and
            ...
            if (cep.best_checked == false) {
              ...
              if (child_node.cmp_to_best_path == 0) {
                ...
                if (old_info.in_best_path) goto handle_best_path_automorphism;
  ...
  if (opt_use_long_prune) long_prune_deallocate();
...
/// @return to-permutation of the vertex ordinals, i.e. `result[i]` is the
/// ordinal of the `i`th input vertex in the canonical order
...
void Digraph::Vertex::remove_duplicate_edges(std::vector<bool>& tmp) {
...
#endif
  for (auto iter = edges_out.begin(); iter != edges_out.end();) {
  ...
  /* Clear tmp */
  for (auto iter = edges_out.begin(); iter != edges_out.end(); iter++) {
    tmp[*iter] = false;
  }

  for (auto iter = edges_in.begin(); iter != edges_in.end();) {
    const unsigned int dest_vertex = *iter;
    if (tmp[dest_vertex] == true) {
      /* A duplicate edge found! */
  ...
  /* Clear tmp */
  for (auto iter = edges_in.begin(); iter != edges_in.end(); iter++) {
    tmp[*iter] = false;
  }
#if defined(BLISS_CONSISTENCY_CHECKS)
  /* Post-conditions  */
...
int Digraph::cmp(Digraph& other) {
  ...
  if (get_nof_vertices() < other.get_nof_vertices()) return -1;
  if (get_nof_vertices() > other.get_nof_vertices()) return 1;
  /* Compare vertex colors */
  for (unsigned int i = 0; i < get_nof_vertices(); i++) {
    if (vertices[i].color < other.vertices[i].color) return -1;
    if (vertices[i].color > other.vertices[i].color) return 1;
  }
  ...
  for (unsigned int i = 0; i < get_nof_vertices(); i++) {
    if (vertices[i].nof_edges_in() < other.vertices[i].nof_edges_in())
      return -1;
    if (vertices[i].nof_edges_in() > other.vertices[i].nof_edges_in()) return 1;
...
Digraph* Digraph::permute(const std::vector<unsigned int>& perm) const {
  ...
  for (unsigned int i = 0; i < get_nof_vertices(); i++) {
    ...
    for (auto ei = v.edges_out.begin(); ei != v.edges_out.end(); ei++) {
...
std::uint64_t Digraph::get_hash64() {
  remove_duplicate_edges();
  sort_edges();

  static_assert(sizeof(std::size_t) == sizeof(std::uint64_t));

  std::size_t h = sequant_boost::hash_value(get_nof_vertices());
  h = std::accumulate(
      vertices.begin(), vertices.end(), h, [](auto h, const auto& vertex) {
        sequant_boost::hash_combine(h, sequant_boost::hash_value(vertex.color));
        sequant_boost::hash_combine(
            h, sequant_boost::hash_value(vertex.edges_out.size()));
        h = std::accumulate(vertex.edges_out.begin(), vertex.edges_out.end(), h,
                            [](auto h, auto edge_dest) {
                              sequant_boost::hash_combine(
                                  h, sequant_boost::hash_value(edge_dest));
                              return h;
                            });
        return h;
      });
...
Digraph* Digraph::read_dimacs(FILE* const fp, FILE* const errstr) {
  ...
  if (nof_vertices <= 0) {
    if (errstr) fprintf(errstr, "error: no vertices\n");
    goto error_exit;
  }
  if (verbose) {
  ...
  for (unsigned int i = cell->length; i > 0; i--) {
    ...
    for (unsigned int j = v.nof_edges_out(); j != 0; j--) {
      ...
      if (ival > neighbour_cell->max_ival) {
        ...
        if (ival == 1) neighbour_heap.insert(neighbour_cell->first);
...
bool Digraph::is_equitable() const {
  ...
  for (Partition::Cell* cell = p.first_cell; cell; cell = cell->next) {
    ...
    for (auto ei = first_vertex.edges_out.begin();
    ...
    for (unsigned int i = cell->length; i > 1; i--) {
      ...
      for (auto ei = vertex.edges_out.begin(); ei != vertex.edges_out.end();
           ei++) {
  ...
  for (Partition::Cell* cell = p.first_cell; cell; cell = cell->next) {
    ...
    for (auto ei = first_vertex.edges_in.begin();
    ...
    for (unsigned int i = cell->length; i > 1; i--) {
      const Vertex& vertex = vertices[*ep++];
      for (auto ei = vertex.edges_in.begin(); ei != vertex.edges_in.end();
           ei++) {
        other_count[p.get_cell(*ei)->first]++;
      }
      for (Partition::Cell* cell2 = p.first_cell; cell2; cell2 = cell2->next) {
...
bool Digraph::is_automorphism(unsigned int* const perm) {
...
#if defined(BLISS_CONSISTENCY_CHECKS)
  if (!is_permutation(get_nof_vertices(), perm)) _INTERNAL_ERROR();
#endif
  ...
  for (unsigned int i = 0; i < get_nof_vertices(); i++) {
    ...
    edges1.clear();
    for (auto ei = v1.edges_in.begin(); ei != v1.edges_in.end(); ei++)
      edges1.insert(perm[*ei]);
    edges2.clear();
    for (auto ei = v2.edges_in.begin(); ei != v2.edges_in.end(); ei++)
      edges2.insert(*ei);
    if (!(edges1 == edges2)) return false;

    edges1.clear();
    for (auto ei = v1.edges_out.begin(); ei != v1.edges_out.end(); ei++)
      edges1.insert(perm[*ei]);
    edges2.clear();
    for (auto ei = v2.edges_out.begin(); ei != v2.edges_out.end(); ei++)
      edges2.insert(*ei);
    if (!(edges1 == edges2)) return false;
  }
...
bool Digraph::nucr_find_first_component(const unsigned int level,
  ...
  for (unsigned int i = 0; i < comp.size(); i++) {
    ...
    for (unsigned int j = v.nof_edges_out(); j > 0; j--) {
      ...
      //   continue;
      if (neighbour_cell->max_ival_count == 0) neighbours.push(neighbour_cell);
      neighbour_cell->max_ival_count++;
    }
...
void Graph::write_dimacs(FILE* const fp) {
  ...
  for (unsigned int i = 0; i < get_nof_vertices(); i++) {
    Vertex& v = vertices[i];
    for (auto ei = v.edges.begin(); ei != v.edges.end(); ei++) {
...
int Graph::cmp(Graph& other) {
  ...
  other.remove_duplicate_edges();
  for (unsigned int i = 0; i < get_nof_vertices(); i++) {
    if (vertices[i].nof_edges() < other.vertices[i].nof_edges()) return -1;
    if (vertices[i].nof_edges() > other.vertices[i].nof_edges()) return 1;
  }
  /* Compare edges */
  for (unsigned int i = 0; i < get_nof_vertices(); i++) {
...
int Graph::cmp(const Graph& other) const {
  /* Compare the numbers of vertices */
  if (get_nof_vertices() < other.get_nof_vertices()) return -1;
  if (get_nof_vertices() > other.get_nof_vertices()) return 1;
  /* Compare vertex colors */
...
void Graph::write_dot(FILE* const fp) {
  ...
  for (std::vector<Vertex>::iterator vi = vertices.begin();
    ...
    for (auto ei = v.edges.begin(); ei != v.edges.end(); ei++) {
      ...
      if (vnum2 > vnum) fprintf(fp, "v%u -- v%u\n", vnum, vnum2);
    }
  }
...
std::uint64_t Graph::get_hash64() {
  ...
  for (std::size_t v = 0; v != vertices.size(); ++v) {
    ...
    sequant_boost::hash_combine(h, sequant_boost::hash_value(vertex.color));
    sequant_boost::hash_combine(h,
                                sequant_boost::hash_value(vertex.edges.size()));
    h = std::accumulate(vertex.edges.begin(), vertex.edges.end(), h,
                        [v](auto h, auto edge_dest) {
                          assert(edge_dest != v);
                          if (edge_dest > v)
                            sequant_boost::hash_combine(
                                h, sequant_boost::hash_value(edge_dest));
...
unsigned int Graph::selfloop_invariant(const Graph* const g,
  ...
  for (auto ei = vertex.edges.begin(); ei != vertex.edges.end(); ei++) {
    if (*ei == v) return 1;
  }
  return 0;
}
...
bool Graph::is_equitable() const {
  ...
  for (Partition::Cell* cell = p.first_cell; cell; cell = cell->next) {
    ...
    for (auto ei = first_vertex.edges.begin(); ei != first_vertex.edges.end();
         ei++) {
      first_count[p.get_cell(*ei)->first]++;
    }
...
bool Graph::is_automorphism(unsigned int* const perm) {
  ...
  for (unsigned int i = 0; i < get_nof_vertices(); i++) {
    Vertex& v1 = vertices[i];
    edges1.clear();
    for (auto ei = v1.edges.begin(); ei != v1.edges.end(); ei++)
      edges1.insert(perm[*ei]);

    Vertex& v2 = vertices[perm[i]];
    edges2.clear();
    for (auto ei = v2.edges.begin(); ei != v2.edges.end(); ei++)
      edges2.insert(*ei);

# original: https://github.com/pytorch/pytorch/blob/master/cmake/Modules/FindMKL.cmake
# changes: adapted for C++ and MacOS
...
IF (NOT MKL_FOUND)
    ...
    IF (TBB_FOUND)
        SET(MKL_THREADING_DEFAULT TBB)
    ELSE()
        SET(MKL_THREADING_DEFAULT SEQ)
    ENDIF()

    SET(MKL_THREADING "${MKL_THREADING_DEFAULT}" CACHE STRING "MKL flavor: SEQ, TBB or OMP")

    if (MKL_THREADING STREQUAL TBB AND NOT TBB_FOUND)
    ...
    # Checks
    CHECK_TYPE_SIZE("void*" SIZE_OF_VOIDP LANGUAGE CXX)
    ...
    IF (EXISTS ${INTEL_MKL_DIR})
        ...
        MESSAGE(STATUS "${INTEL_MKL_DIR}/lib/${mklvers}")
        IF (MSVC)
            SET(CMAKE_LIBRARY_PATH ${CMAKE_LIBRARY_PATH}
    ...
    IF (MKL_LIBRARIES)
        ...
        ELSEIF (CMAKE_CXX_COMPILER_ID AND NOT CMAKE_CXX_COMPILER_ID STREQUAL "Intel")
            FIND_LIBRARY(MKL_KERNEL_libirc "irc")
        ENDIF ()
